...
- 右侧检查器：编辑选中对象；按 Enter、移出输入框、保存或启动预览时会自动提交合法修改。“应用属性”仍可用于明确提交。
- 右下“诊断”：同时显示 Flow 模型错误、Graph 错误和布局/动画资产闭包错误。

顶部的“重新启动预览”会先提交当前检查器，再从所选 Entry 重新创建生产 `UIFlowRuntime`。修改 Flow 后，
运行控制会进入“预览已过期”状态，防止把旧预览误认为最新结果；重新启动后恢复。
...
- “保存”写回当前文件；“另存为”创建新文件。
- 保存和预览都会先提交当前检查器；非法数字、Scope、输入策略或中断策略会保留在输入框中并阻止操作，
  不会再静默改成默认值。
- 当前 Flow 有未保存修改时，工具栏“打开”不会覆盖它；先保存或关闭当前 Flow。
