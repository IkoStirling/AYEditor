...
bool EditorPlayRuntime::seedPlayerControllerLogia() {
    ...
    const std::string sourcePath = (std::filesystem::path(_engineAssetsRoot)
        / "AYScript" / "templates" / "player_controller.logia").string();
    ...
    if (ayt::io::File::exists(destPath)) {
        const std::string existing = ayt::io::File::readAllText(destPath);
        // One-time migration for the broken template previously generated by
        // AYEditor. Preserve every other existing script, including user
        // edits, instead of treating the cache as disposable on each launch.
        const bool isLegacyGeneratedTemplate =
            existing.find("if vec2.length(move) > 0.1") != std::string::npos
            && existing.find("self.apply_move(vec2.normalized(move), dt)")
                != std::string::npos;
        if (!isLegacyGeneratedTemplate) return true;
        std::fprintf(stderr,
            "[EditorPlayRuntime] migrating generated PlayerController.logia "
            "to the compiler-tested template\n");
    }
...
bool EditorPlayRuntime::seedSkyBoxPng() {
    ...
    const std::string sourcePath = (std::filesystem::path(_engineAssetsRoot)
        / "AYRenderer" / "textures" / "skyBox.png").string();
    if (!fileExists(sourcePath)) {
        return false;
    }
